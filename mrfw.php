 ...
 * Description: A simple and easy way to manually add WooCommerce product reviews from your WordPress dashboard.
 * Version: 1.0.1
 * Author: Royalz Store
 * Author URI: https://www.royalz.store
		...
		*/
		...
		function render_metabox( $post ) {
	        ...
	        <div class="woocommerce_options_panel" id="mrfw_form">
				...
				<p class="form-field">
					<label for="mrfw_form_date"><?php esc_html_e('Date', 'mrfw'); ?></label>
					<?php $date = new DateTime(); ?>
					<input type="datetime-local" value="<?php echo $date->format('Y-m-d\TH:i'); ?>" name="mrfw_form_date" id="mrfw_form_date">
				</p>
				<p class="form-field">
		...
		function insert_comment() {
			...
			// Return error if comment isn't inserted
			if ( $comment_id == '-1' ) {
				wp_die( -1 );
			}
			// Mark review approved if necerssary
			if ( $approved ) {
